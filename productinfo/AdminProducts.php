...
class AdminProducts extends AdminTab
{
				...
				if (Validate::isLoadedObject($object))
				{
					...
					if ($object->update())
					{
						...
						elseif ($id_image = $this->addProductImage($object, Tools::getValue('resizer')))
						{
							...
							if ($_extra = Tools::getValue('extra')) {
								$object->extra = $_extra;
							}
	...
	function displayFormInformations($obj, $currency)
	{
		...
		/* Extra product info module v1.0 */
		$_sql = "SELECT * from `"._DB_PREFIX_."product_info_fields` ORDER by `name` asc";
		$_result = Db::getInstance()->ExecuteS($_sql);
		$_result = ($_result ? $_result : array());
		echo '		<tr>
						<td class="col-left" colspan="2" style="padding: 5px 10px 0 0;"><b>'.$this->l('Extra product information').'</b><hr /></td>
					</tr>';
		foreach ($_result as $field => $data) {
			echo '	<tr>
						<td class="col-left">'.$data['label'].':</td>
						<td style="padding-bottom:5px;"><div class="translatable">';
			foreach ($this->_languages as $language) {
				echo '		<div class="lang_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').'; float: left;">
								<input type="text" id="'.$data['name'].'_'.$language['id_lang'].'" name="extra['.$data['name'].']['.$language['id_lang'].']" value="'.(isset($obj->extra[$data['name']][$language['id_lang']]) ? $obj->extra[$data['name']][$language['id_lang']] : '').'" />
							</div>';
			}
			echo '		</div>';
			echo '		<p class="clear">' . $data['help_text'] . '</p>';
			echo '		</td>
					</tr>';
		}
		if (empty($_result)) {
			echo '	<tr>
						...
						' . $this->l('No extra fields have been created.') . '
						</td>
					</tr>
			';
		}
		echo '
				<tr>
					<td class="col-left" style="padding-bottom: 5px;" colspan="2">
						<a href="index.php?tab=AdminModules&configure=productinfo&token=' . Tools::getAdminTokenLite('AdminModules') . '&add=field&redirect='.urlencode($currentIndex.'&updateproduct&token='.Tools::getAdminTokenLite('AdminCatalog')).'" title="' . $this->l('Add new field') . '"><img src="' . _PS_ADMIN_IMG_ . 'add.gif" alt="" /> ' . $this->l('Add new field') . '</a>
					</td>
				</tr>
				<tr>
					<td class="col-left" style="padding-bottom:5px;padding-right:10px;" colspan="2">
						<div class="hint clear" style="display:block;">
							<a href="index.php?tab=AdminModules&configure=productinfo&token=' . Tools::getAdminTokenLite('AdminModules') . '#bulkUpdateForm" title="' . $this->l('Bulk update') . '">'.$this->l('You can do a bulk update on these extra settings. Click here.').'</a>
						</div>
					</td>
				</tr>
			';
